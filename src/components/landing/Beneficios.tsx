export function Beneficios() {
  const benefits = [
    { icon: "🎯", title: "Foco no exame nacional", text: "Os exercícios e a forma de resposta são treinados no formato exato do IAVE, não no formato do manual." },
    ...
    <section className="py-16 bg-paper-2 border-y border-border">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <p className="text-[0.92rem] text-ink-2 leading-[1.78] max-w-xl mb-8">
          O objetivo não é apenas aprovar. É que o aluno perceba a matéria de uma forma que lhe dê autonomia e resultados mensuráveis.
        </p>

        <div className="grid md:grid-cols-3 gap-4">
          {benefits.map((b) => (
            <div key={b.title} className="group bg-card border border-border rounded-xl p-5 hover:shadow-[0_6px_26px_oklch(0.43_0.09_185/0.09)] hover:-translate-y-0.5 transition-all">
              <div className="text-[1.3rem] mb-2 transition-transform group-hover:scale-110 origin-left inline-block">{b.icon}</div>
